...
# Token-ka waxaa laga akhrinayaa Environment Variables-ka Kinesis Cloud
...
async def on_message(message):
    ...
    if isinstance(message.channel, discord.DMChannel):
        ...
        # Badanka 2: Contact Team (Lagu qariyay link-gaaga)
        contact_url = "https://discord.com/users/1388255325345419409"
        view.add_item(discord.ui.Button(label="Contact Team 👤", url=contact_url))
        
        await message.channel.send(
            f"Hi {message.author.name} I'm Bot\nPlease use / This I'm Working!",
            view=view
        )

# --- 2. MODERATION (EPHEMERAL) ---
...
@bot.tree.command(name="setwelcome", description="Habee welcome-ka")
async def setwelcome(interaction: discord.Interaction, channel: discord.TextChannel, message: str):
    ...
    save_welcome(data)
    await interaction.response.send_message(f"✅ Welcome-ka waa la dhigay.", ephemeral=True)
...
async def on_member_join(member):
    ...
    if g_data:
        ...
        msg = g_data["message"].replace("{user}", member.mention).replace("{server}", member.guild.name)
...
async def help_cmd(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("Isticmaal `/` si aad u aragto amarrada.", view=view, ephemeral=True)
